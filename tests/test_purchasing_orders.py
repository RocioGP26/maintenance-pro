...
from app.inventario_comercial.service import registrar_pago_compra
...
class TestPurchasingOrders(unittest.TestCase):
    ...
    def test_partial_then_total_receipt_updates_stock_once(self):
        ...
        first = registrar_recepcion(self.empresa.id, self.user.id, order, {"idempotency_key": "receipt-1", "fecha": date(2026, 7, 11), "fecha_vencimiento": date(2026, 8, 10), "documento_proveedor": "FAC-001"}, {line.id: (1, 0, "")})
        db.session.commit()
        self.assertEqual(first.estado, "confirmada"); self.assertEqual(order.estado, "parcial"); self.assertEqual(self.product.stock, 1)
        self.assertEqual(first.compra.total, 50); self.assertEqual(first.compra.fecha_vencimiento, date(2026, 8, 10)); self.assertEqual(first.compra.estado_pago, "pendiente")
        same = registrar_recepcion(self.empresa.id, self.user.id, order, {"idempotency_key": "receipt-1", "fecha": date(2026, 7, 11)}, {line.id: (1, 0, "")})
        self.assertEqual(same.id, first.id); self.assertEqual(self.product.stock, 1)
        second = registrar_recepcion(self.empresa.id, self.user.id, order, {"idempotency_key": "receipt-2", "fecha": date(2026, 7, 11)}, {line.id: (1, 0, "")})
        db.session.commit()
        self.assertEqual(second.estado, "confirmada"); self.assertEqual(order.estado, "recibida"); self.assertEqual(self.product.stock, 2)
        registrar_pago_compra(first.compra, monto=20, fecha=date(2026, 7, 12)); db.session.commit()
        self.assertEqual(first.compra.estado_pago, "parcial"); self.assertEqual(first.compra.saldo_pendiente, 30)
        content, name = export_cxp_excel(self.empresa, [first.compra, second.compra], usuario=self.user)
        self.assertTrue(name.endswith(".xlsx")); self.assertEqual(load_workbook(BytesIO(content)).active["A7"].value, "Documento")

    def test_rejection_requires_reason_and_does_not_touch_stock(self):
        order = self._sent_order(); line = order.lineas[0]
        with self.assertRaisesRegex(ValueError, "motivo"):
            registrar_recepcion(self.empresa.id, self.user.id, order, {"idempotency_key": "reject-bad", "fecha": date(2026, 7, 11)}, {line.id: (0, 1, "")})
        self.assertEqual(len(order.recepciones), 0)
        receipt = registrar_recepcion(self.empresa.id, self.user.id, order, {"idempotency_key": "reject-ok", "fecha": date(2026, 7, 11)}, {line.id: (0, 1, "Daño visible")})
        db.session.commit()
        self.assertEqual(receipt.estado, "rechazada"); self.assertIsNone(receipt.compra); self.assertEqual(order.estado, "parcial"); self.assertEqual(self.product.stock, 0)
