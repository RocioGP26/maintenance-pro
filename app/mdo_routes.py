...
mdo_bp = Blueprint("mdo", __name__, url_prefix="/mdo")

_ROOT = Path(__file__).resolve().parent.parent / "docs" / "mdo"
...
@mdo_bp.route("/css/<path:filename>")
...
@mdo_bp.route("/chapters/<path:filename>")
def chapters(filename: str):
    ...
    if filename.endswith(".md"):
        return send_from_directory(ch_dir, filename, mimetype="text/markdown; charset=utf-8")
    return send_from_directory(ch_dir, filename)


@mdo_bp.route("/<path:filename>")
...
@mdo_bp.route("")
def index_no_slash():
    return redirect(url_for("mdo.index"))
