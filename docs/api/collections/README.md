...
| `maintix-sandbox.postman_environment.json` | Postman · entorno Sandbox |
...
1. Import → `maintix-api-v1.postman_collection.json`
2. Import → `maintix-sandbox.postman_environment.json`
3. Seleccionar entorno **Maintix Sandbox**
4. Ejecutar **Authentication → Login** (ajustar credenciales)
5. El script guarda `token` automáticamente

### Insomnia

1. Application → Import → `maintix-api-v1.insomnia.json`
...
```bash
openapi2postmanv2 -s ../openapi.v1.yaml -o maintix-api-v1.postman_collection.json
```
